...
export default function PaginatedReviewList({ reviews, searchData }) {
  const hasSearchData = Array.isArray(searchData) && searchData.length > 0;
  const dataForRender = hasSearchData ? searchData : reviews || [];

  return (
    <div className="space-y-4">
      <div className="space-y-4">
        {dataForRender.map((comment) => (
          <Link
